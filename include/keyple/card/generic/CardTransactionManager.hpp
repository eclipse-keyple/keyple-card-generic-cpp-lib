...
public:
     ...
     * @since 2.0.0
     */
    virtual CardTransactionManager& prepareApdu(const std::string& apduCommand)
        = 0;
     ...
     * processApdusToByteArrays(ChannelControl) is invoked.
     *
     * @param apduCommand A not empty const uint8_t arrays containing raw APDU
     *        commands.
     * @return This instance.
     * @since 2.0.0
     */
    virtual CardTransactionManager&
    prepareApdu(const std::vector<uint8_t>& apduCommand)
        = 0;
     ...
     * unspecified, null if no output data is expected.
     * @return This instance.
     * @since 2.0.0
     */
    ...
    /**
     * Prepares an APDU to be transmitted the next time
     * processApdusToByteArrays() is invoked.
     *
     * @param cla The class const uint8_t.
     * @param ins The instruction const uint8_t.
     * @param p1 The P1 parameter.
    ...
    /**
     * Transmits all prepared APDUs, closes the physical channel if required,
     * and returns a list of responses to the APDUs in the form of list of of
     ...
     * @throw TransactionException If the communication with the card or the
     * reader has failed.
     */
    virtual const std::vector<std::vector<uint8_t>>
    processApdusToByteArrays(ChannelControl channelControl) = 0;

    /**
     * Transmits all prepared APDUs, closes the physical channel if required,
     * and returns a list of responses to the APDUs in the form of list of
     * hexadecimal strings.
     *
     * <p>If the prepared APDU list is empty an empty list is returned.
     *
    ...
    processApdusToHexStrings(ChannelControl channelControl) = 0;
};
