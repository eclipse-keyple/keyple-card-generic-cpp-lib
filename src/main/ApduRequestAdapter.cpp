...
: mApdu(apdu)
, mSuccessfulStatusWords({ApduRequestAdapter::DEFAULT_SUCCESSFUL_CODE})
{
}
...
ApduRequestAdapter::getApdu() const
{
    return mApdu;
}

void
ApduRequestAdapter::setApdu(const std::vector<std::uint8_t>& apdu)
{
    mApdu = apdu;
}

const std::vector<int>&
ApduRequestAdapter::getSuccessfulStatusWords() const
{
    return mSuccessfulStatusWords;
}

const std::string&
ApduRequestAdapter::getInfo() const
{
    return mInfo;
}

std::ostream&
operator<<(std::ostream& os, const std::shared_ptr<ApduRequestAdapter> ara)
{
